...
NOME_MODELO_CLASSIFICACAO = 'gemini-1.5-flash-latest'
VERSAO_PROMPT_CLASSIFICACAO = 'v1.2-classificacao-chunk'  # Atualize conforme necessário
...
def classificar_chunk_gemini(texto, gemini_keys, gemini_key_names, model, current_key_index=0):
    """
    Classifica um chunk de texto usando a API Gemini, retornando:
    - classificacao_acesso (0, 1, 2)
    - justificativa_acesso (str)
    - confianca_classificacao (float)
    - nome_modelo_classificacao (str)
    - versao_prompt_classificacao (str)
    """
    logger = logging.getLogger(__name__)
    prompt = f"""
        ...
        Antes de classificar, siga estes 3 princípios fundamentais:
        ...
        1.  CLASSIFICACAO: Um número inteiro (0, 1 ou 2) para indicar a classificação de acesso:
            - 0 (ACESSO RESTRITO): Documento protegido e acessível apenas por indivíduos com autorização legal específica.
                ...
                * **Informações de processos de apuração, sindicância ou disciplinares (PAD), cujo sigilo é essencial para garantir a eficácia da investigação e o direito à ampla defesa.** (MOTIVO DA MUDANÇA: Adiciona a regra explícita que faltava para o "Relatório de Investigação").
            ...
            - 1 (ACESSO INTERNO): Visível para o público interno, mas não para o externo.
                Aplica-se a:
                * **Atos administrativos preparatórios, como Pareceres Jurídicos, Notas Técnicas, Relatórios de Análise de Risco e Despachos que fundamentam uma decisão futura.** (MOTIVO DA MUDANÇA: Adiciona a regra explícita que faltava para o "Parecer Jurídico").
            ...
            - 2 (ACESSO PÚBLICO): Visível para toda a Internet.
                ...
                * **Importante: Esta classificação NÃO se aplica a documentos preparatórios, mesmo que tratem de temas que se tornarão públicos.** (MOTIVO DA MUDANÇA: Reforça a regra do nível 1).
        ...
        Responda OBRIGATORIAMENTE no formato EXATO abaixo, sem nenhum texto adicional antes ou depois:
    ...
    """
    tentativas = 0
    total_chaves = len(gemini_keys)
    chave_inicial = current_key_index
    while tentativas < total_chaves:
        try:
            logger.info(f"Usando chave Gemini: {gemini_key_names[current_key_index]}")
                    ...
                    if linha.upper().startswith("CLASSIFICACAO:"):
                        try:
                            ...
                            if valor_int in [0, 1, 2]:
                                ...
                                logger.warning(f"Classificação inválida recebida: {valor_str}. Esperado 0, 1 ou 2.")
                                classificacao = -1
                                explicabilidade = f"Erro: Classificação '{valor_str}' inválida recebida do modelo."
                                break
                        except ValueError:
                            logger.warning(f"Não foi possível converter classificação para inteiro: {linha.split(':', 1)[1].strip()}")
                            classificacao = -1
                            explicabilidade = "Erro: Formato da classificação inválido."
                            break
                        except Exception as e_parse:
                            logger.error(f"Erro ao parsear 'CLASSIFICACAO': {str(e_parse)}")
                            classificacao = -1
                            explicabilidade = "Erro ao parsear CLASSIFICACAO."
                            break
                        ...
                        try:
                            ...
                            if not 0.0 <= confianca <= 1.0:
                                logger.warning(f"Confiança fora do intervalo [0.0, 1.0]: {confianca}")
                                confianca = max(0.0, min(1.0, confianca))
                        except ValueError:
                            logger.warning(f"Não foi possível converter confiança para float: {linha.split(':', 1)[1].strip()}")
                            confianca = 0.0
                        except Exception as e_parse:
                            logger.error(f"Erro ao parsear 'CONFIANCA': {str(e_parse)}")
                            confianca = 0.0
            else:
                logger.warning(f"Resposta do Gemini em formato inesperado: {resposta}")
                classificacao = -1
            ...
            if classificacao == -1:
                logger.error(f"Classificação inválida recebida ou erro de parsing. Tentando próxima chave se houver. Justificativa: {explicabilidade}")
                tentativas += 1
                current_key_index = (current_key_index + 1) % total_chaves
                if current_key_index == chave_inicial:
                    logger.error("Todas as chaves Gemini foram testadas e nenhuma retornou resposta válida. Motivo: erro de parsing/classificação inválida.")
                    return -1, explicabilidade, 0.0, NOME_MODELO_CLASSIFICACAO, VERSAO_PROMPT_CLASSIFICACAO
                model = model.__class__(gemini_keys[current_key_index])
                continue
            return classificacao, explicabilidade, confianca, NOME_MODELO_CLASSIFICACAO, VERSAO_PROMPT_CLASSIFICACAO
        except Exception as e:
            if "quota" in str(e).lower() or "rate limit" in str(e).lower() or "blocked" in str(e).lower():
                logger.warning(f"Quota/Rate limit/bloqueio excedida para a chave {gemini_key_names[current_key_index]}. Trocando para a próxima chave...")
                tentativas += 1
                current_key_index = (current_key_index + 1) % total_chaves
                if current_key_index == chave_inicial:
                    logger.error("Todas as chaves Gemini atingiram o limite de quota/bloqueio. Nenhuma chave disponível para uso.")
                    return -1, f"QUOTA_EXCEDIDA_TODAS_CHAVES: {str(e)}", 0.0, NOME_MODELO_CLASSIFICACAO, VERSAO_PROMPT_CLASSIFICACAO
                model = model.__class__(gemini_keys[current_key_index])
                continue
            logger.error(f"Erro ao chamar API do Gemini: {str(e)}")
            return -1, f"Erro ao chamar API do Gemini: {str(e)}", 0.0, NOME_MODELO_CLASSIFICACAO, VERSAO_PROMPT_CLASSIFICACAO
    logger.error("Todas as chaves Gemini atingiram o limite de quota ou falharam por erro de parsing/classificação inválida.")
    return -1, "QUOTA_EXCEDIDA_TODAS_CHAVES ou erro de parsing/classificação inválida", 0.0, NOME_MODELO_CLASSIFICACAO, VERSAO_PROMPT_CLASSIFICACAO
        ...
        while True:
            ...
            for chunk in chunks:
                try:
                    ...
                    classificacao, justificativa, confianca, nome_modelo, versao_prompt = classificar_chunk_gemini(
                        chunk["chunk_texto"],
                        gemini_keys,
                        gemini_key_names,
                        model,
